...
our $NAME_RE        = qr/ [_a-z]  \w* /ix;
our $MACRO_PARAM_RE = qr/ [_a-z#] \w* /ix;
our $LABEL_RE       = qr/ (?| ^ 		     (?<label> $NAME_RE) (?: \s+ | \s* : \s* )
		  | ^ \s* \. \s* (?<label> $NAME_RE) \s+
		  | ^ \s*        (?<label> $NAME_RE) \s* : \s*
		  ) /ix;
our $OPT_LABEL_RE = qr/ ^ (?<label_field> $LABEL_RE | \s+ ) /ix;
our $QSTR_RE      = qr/ (?| ' (?<str> [^']* ) ' 
		  | " (?<str> [^"]* ) " 
		  ) /ix;
our $QFILE_RE = qr/ (?| ' ( [^']+ ) ' 
...
sub read_file_it {
    ...
    return imap { { text => $_->[1], file => $path, line_nr => 1 + $_->[0] } }
    ienumerate imap { s/\s+$//; $_ }
...
sub add_label_suffix {
    ...
    return imap {
        for ( $_->{text} ) {
            if ( $_ =~
                /^\s*(IF|IFDEF|IFNDEF|ELSE|ELIF|ELIFDEF|ELIFNDEF|ENDIF)/i )
            {
                next;
            }
            s/^(\w+)\s+(\w+)/$1: $2/;
...
sub parse_include_it {
    ...
    return iflatten sub {
        ...
        if ( $line->{text} =~ /^ \s* [\#\*]? \s* INCLUDE \s+ $QFILE_RE /ix ) {
...
sub parse_macros_it {
    ...
    return iter sub {
        while (1) {
            defined( my $line = <$in> ) or return;
            if ( $line->{text} =~ /^ $LABEL_RE \b MACRO \b (?<args> .*)/ix ) {
                ...
                while (1) {
                    ...
                    if (
                        $macro_line->{text} =~ /^ \s+ LOCAL \b (?<args> .*)/ix )
                    {
                        push @local,
                            extract_macro_params( $+{args}, $macro_line );
                    }
                    else {
                ...
                # save macro
                $MACRO{ uc($name) }
                    and error( $line, "macro multiply defined" );
                $MACRO{ uc($name) } = {
                    args  => \@args,
                    local => \@local,
                    lines => \@lines,
...
sub expand_macros_it {
    my ($in) = @_;
    return iflatten imap {
        if (
            $_->{text} =~ /^ $OPT_LABEL_RE \b
				    (?<name> $NAME_RE) \b 
				    (?<args> .*) $/ix && defined( $MACRO{ uc( $+{name} ) } )
            )
        {
...
sub expand_defl_it {
    my ($in) = @_;
    return imap {
        if ( $_->{text} =~ /^ $LABEL_RE \b DEFL \b \s* (?<expr> .*)/ix ) {
...
sub parse_directives_it {
    ...
    return iter sub {
        while (1) {
            ...
            if ( $line->{text} =~ /^ $OPT_LABEL_RE \b END \b/ix ) {
...
while ( @ARGV && $ARGV[0] =~ /^-/ ) {
    ...
    if    (/^-I(.*)/)                 { add_path($1); }
        ...
        $_ = shift;
        push @OPTIONS, "-o$_";
    }    # remove <space> between -o and file name
