...
string Options::bin_filename(const string& filename, const string& section) {
    ...
    string filename1 = prepend_output_dir(
                           file_path.lexically_normal().generic_string());
    string filename2 = replace_ext(filename1,
                                   file_ext.lexically_normal().generic_string());
    file_path = filename2;
...
string Options::norm_filename(const string& filename) {
    ...
    if (cur_path.size() > 2 && isalpha(cur_path[0]) && cur_path[1] == ':' &&
            filename.size() > 3 && filename[0] == '/' && isalpha(filename[1])
            && filename[2] == '/') {
...
bool Options::parse_opt_int(int& value, const string& opt_arg) {
    ...
    else if (opt_arg.length() > 2 && opt_arg[0] == '0'
             && tolower(opt_arg[1]) == 'x') {
        p += 2;
        radix = 16;
    }
...
void Options::set_origin(const string& opt_arg) {
    int value = 0;
    if (!parse_opt_int(value, opt_arg)
            || value < 0) {	// value can be >0xffff for banked address
...
void Options::expand_list_glob(const string& pattern_) {
    ...
    for (auto& file : files) {
        if (fs::is_regular_file(file)) {
            ...
            g_options.include_path.push_back(
                file.parent_path().lexically_normal().generic_string());
            {
...
bool Options::search_source(const string& filename, string& out_filename) {
    if (str_ends_with(filename,
        ...
        string asm_filename = m4_full_path.substr(0,
                              m4_full_path.size() - strlen(EXT_M4));	// file.asm
        string m4_cmd = "m4 " + m4_options + " \"" + m4_full_path + "\" > \"" +
                        asm_filename + "\"";
        if (verbose) {
            cout << "% " << m4_cmd << endl;
        }
