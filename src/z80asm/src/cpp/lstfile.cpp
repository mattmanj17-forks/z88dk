...
void LstFile::source_line(Location location, const string& section, int asmpc,
                          int phased_pc, const string& text) {
    if (m_ofs.is_open()) {
        out_line();
...
void LstFile::expanded_line(const string& section, int asmpc, int phased_pc,
                            const string& text) {
    if (m_ofs.is_open()) {
        out_line();
...
void LstFile::patch_bytes(const string& section, int asmpc,
...
int LstFile::num_rows() {
    return (static_cast<int>(m_bytes.size()) + BytesWidth / 2 - 1) /
           (BytesWidth / 2);
}
...
void list_source_line(const char* filename, int line_num, const char* section,
    ...
    g_list_file.source_line(Location(filename, line_num), section, asmpc, phased_pc,
                            text);
}
...
void list_patch_bytes(const char* section, int asmpc, int value,
                      int num_bytes) {
    if (asmpc >= 0) {
        vector<uint8_t> bytes;
        for (int i = 0; i < num_bytes; i++) {
...
void list_got_source_line(const char* filename, int line_num,
                          const char* text) {
    if (filename && list_is_on()) {
        list_source_line(filename, line_num,
                         get_cur_section_name(), get_PC(),
                         get_phased_PC() >= 0 ? get_phased_PC() : get_PC(),
                         text);
    }
}
