...
public:
    ...
    void close();
    void source_line(Location location, const string& section, int asmpc,
                     int phased_pc, const string& text);
    void expanded_line(const string& section, int asmpc, int phased_pc,
                       const string& text);
    void append_bytes(const vector<uint8_t>& bytes);
    void patch_bytes(const string& section, int asmpc,
                     const vector<uint8_t>& bytes);
    void end_line();
    void set_list_on(bool f = true) {
        m_list_on = f;
    }
    bool list_is_on() const {
        return m_list_on;
    }
