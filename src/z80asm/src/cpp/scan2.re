...
Token::Token(TType type, bool blank_before, int ivalue)
    : m_type(type), m_blank_before(blank_before), m_ivalue(ivalue),
      m_keyword(Keyword::None) {
}

Token::Token(TType type, bool blank_before, double fvalue)
    : m_type(type), m_blank_before(blank_before), m_fvalue(fvalue),
      m_keyword(Keyword::None) {
}
...
void ScannedLine::append(const ScannedLine& other) {
    ...
    std::copy(other.tokens().begin(), other.tokens().end(),
...
bool FileScanner::fill() {
    if (m_got_eof) {
        ...
        m_buffer.resize(cur_size + FILL_SIZE +
                        YYMAXFILL);      // reserve extra YYMAXFILL for re2c
