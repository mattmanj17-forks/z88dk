...
/* local functions */
static void link_lib_module(const char* modname, obj_file_t* obj,
                            StrHash* extern_syms);
...
static const char* parse_st_str(obj_file_t* obj) {
    ...
    int id = parse_obj_file_int(obj);
    return strtable_lookup(obj->st, id);
}

static obj_file_t* obj_files_append(obj_file_t** plist, const char* filename,
                                    Module1* module) {
    init();
...
static void object_file_reserve_space(obj_file_t* obj, Module1* module) {
    ...
    const char* modname = spool_add(parse_st_str(
                                        obj));     // st from obj from lib will be deleted, save modname
    CURRENTMODULE->modname = modname;

    if (goto_code(obj)) {
        while (true) {
            int code_size = parse_obj_file_int(obj);
            if (code_size < 0) {
                break;
            }
            ...
            int origin = parse_obj_file_int(obj);
            set_origin(origin, section);
            section->align = parse_obj_file_int(obj);

            append_reserve(code_size);

            // align to dword size
            unsigned aligned_size = ((code_size + (sizeof(int32_t) - 1)) & ~(sizeof(
                                         int32_t) - 1));
...
static void read_cur_module_exprs(Expr1List* exprs, obj_file_t* obj) {
    while (true) {
        range_t range = parse_obj_file_int(obj);
        ...
        int line_num = parse_obj_file_int(obj);
...
*/
static int compute_equ_exprs_once(Expr1List* exprs, bool show_error,
        ...
        if (expr->target_name) {
            ...
            if (sym != NULL) {      // was not local label
                ...
                if (!module_relative_addr || /* link phase */
                        (Expr_is_local_in_section(expr, CURRENTMODULE, CURRENTSECTION)
                         &&	/* or symbols from other sections */
                         Expr_without_addresses(
                             expr))		/* expression addressees - needs to be computed at link time */
                   ) {
...
static void check_equ_exprs_solved(Expr1List* exprs,
                                   bool module_relative_addr) {
        ...
        if (!expr->result.not_evaluable) {		/* not unresolved */
            ...
            case RANGE_WORD:
                ...
                if (expr->type == TYPE_ADDRESS) {
                    ...
                    *(intArray_push(expr->section->reloc)) = expr->patch_ptr +
                            get_cur_module_start();
...
static bool module_same_cpu(obj_file_t* obj, int fpos) {
    int fpos0 = obj->i;
    obj->i = fpos;
    cpu_t cpu_id = parse_obj_file_int(obj);
...
static bool linked_module(obj_file_t* obj, StrHash* extern_syms) {
    ...
    if (goto_defined_names(obj)) {
        while (!linked) {
            sym_scope_t scope = parse_obj_file_int(obj);
            if (scope == SCOPE_NONE) {
                break;    // end of list
            }
...
static bool linked_libraries(StrHash* extern_syms) {
    ...
    for (obj_file_t* lib = g_libraries; !linked && lib != NULL; lib = lib->next) {
        ...
        for (; !linked && pos > 0 && pos < lib->size; pos = next_pos) {
            lib->i = pos;
            next_pos = parse_obj_file_int(lib);
            int module_size = parse_obj_file_int(lib);
...
static void link_module(obj_file_t* obj, StrHash* extern_syms) {
    ...
    if (goto_defined_names(obj)) {
        while (true) {
            sym_scope_t scope = parse_obj_file_int(obj);			// scope of symbol
            if (scope == SCOPE_NONE) {							// end marker
                break;
            }

            sym_type_t type = parse_obj_file_int(obj);				// type of symbol
            const char* section_name = parse_st_str(obj);		    // section
            int value = parse_obj_file_int(obj);					// value
            const char* name = parse_st_str(obj);				    // symbol name
            const char* def_filename = parse_st_str(obj);		    // where defined
            int line_num = parse_obj_file_int(obj);					// where defined
            ...
            switch (scope) {
            case SCOPE_LOCAL:
                sym = define_local_sym(name, value, type);
                break;
            case SCOPE_PUBLIC:
                sym = define_global_sym(name, value, type);
                break;
            default:
                error_invalid_object_file(obj->filename);
                return;
...
static void link_lib_module(const char* modname, obj_file_t* obj,
    ...
    Module1* lib_module = set_cur_module(
                              new_module());	// new module to link library
...
void link_modules(void) {
    ...
    // link all .o modules
    for (obj_file_t* obj = g_objects; !get_error_count()
...
void CreateBinFile(void) {
    ...
    if (binfile.fp) {
        if (need_reloc_routine) {
            ...
            *(reloctable + 1) = (byte_t)totaladdr /
                                256U;  /* total of relocation elements */
            *(reloctable + 2) = (byte_t)sizeof_reloctable % 256U;
            *(reloctable + 3) = (byte_t)sizeof_reloctable /
                                256U; /* total size of relocation table elements */
...
static int sym_first(int c) {
    return c == '_' || isalpha(c);
}
static int sym_next(int c) {
...
static void rename_module_local_symbols(Module1* module) {
    ...
    for (sym_it = Symbol1Hash_first(module->local_symtab); sym_it != NULL;
            sym_it = Symbol1Hash_next(sym_it)) {
    ...
    /* change symbol names */
    for (name_it = StrHash_first(old_syms); name_it != NULL;
            name_it = StrHash_next(name_it)) {
    ...
    for (expr_it = Expr1List_first(module->exprs); expr_it != NULL;
            expr_it = Expr1List_next(expr_it)) {
        expr = expr_it->obj;
...
static void merge_local_symbols(StrHash* extern_syms) {
    ...
    first_module = get_first_module(NULL);
    xassert(first_module != NULL);

    for (module = get_first_module(&it); module != NULL;
            module = get_next_module(&it)) {
...
static void create_extern_symbols(StrHash* extern_syms) {
    ...
    for (elem = StrHash_first(extern_syms); elem != NULL;
            elem = StrHash_next(elem)) {
