...
void ExprOp_compute(ExprOp* self, Expr1* expr, bool not_defined_error) {
    ...
    case NUMBER_OP:
        Calc_push(self->d.value);
        break;
    case UNARY_OP:
        Calc_compute_unary(self->d.op->calc.unary);
        break;
    case BINARY_OP:
        Calc_compute_binary(self->d.op->calc.binary);
        break;
    case TERNARY_OP:
        Calc_compute_ternary(self->d.op->calc.ternary);
        break;
...
void Expr1_init(Expr1* self) {
    ...
    self->patch_ptr = get_cur_module_size();	/* BUG_0015 */
    self->opcode_size = 0;
...
long Expr_eval(Expr1* self, bool not_defined_error) {
    ...
    if (self->type == TYPE_COMPUTED && self->is_computed) {
        ...
        for (i = 0; i < ExprOpArray_size(self->rpn_ops); i++) {
            ...
            case SYMBOL_OP:
                type = MAX(type, expr_op->d.symbol->type);
                break;
            case ASMPC_OP:
                type = MAX(type, TYPE_ADDRESS);
                break;
            default:; /* no change */
            }
        }
...
bool Expr_is_local_in_section(Expr1* self, struct Module1* module,
    ...
    for (i = 0; i < ExprOpArray_size(self->rpn_ops); i++) {
        ...
        switch (expr_op->op_type) {
        case SYMBOL_OP:
            if (expr_op->d.symbol->module != module
                    || expr_op->d.symbol->section != section) {
                return false;
            }
            break;
...
bool Expr_without_addresses(Expr1* self) {
    ...
    if (num_addresses > 1 ||	// two or more addresses
            (num_addresses == 1
             && ExprOpArray_size(self->rpn_ops) > 1)) {	// or one address with more operands
        return false;
    }
    else {
        return true;
