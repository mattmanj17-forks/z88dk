...
Section1* new_section( const char* name ) {
    ...
    if ( g_cur_section == NULL ) {
        ...
        if ( g_default_section != NULL && *name != '\0' ) {
            ...
            if ( last_id >= 0 ) {
                intArray_item( g_cur_section->module_start,
                               last_id );    /* init [0..module_id] to zero */
            }
        }
    }
...
int get_cur_module_start( void ) {
    return section_module_start( g_cur_section, g_cur_module );
}
int get_cur_module_size(  void ) {
    return section_module_size(  g_cur_section, g_cur_module );
}
int get_cur_opcode_size(void) {
    init_module();
    return g_cur_section->opcode_size;
}
...
void sections_alloc_addr(void) {
    ...
    for (section = get_first_section(&iter); section != NULL;
        ...
        if (next_section != NULL && !(next_section->origin >= 0)
                && next_section->align > 1
...
static byte_t* alloc_space( int addr, int num_bytes ) {
    ...
    /* reserve space */
    if ( num_bytes > 0 ) {
        (void)   ByteArray_item( g_cur_section->bytes,
                                 base_addr + addr + num_bytes - 1 );
...
void patch_byte( int addr, byte_t byte1 ) {
    patch_value( addr, byte1, 1 );
}
void patch_word( int addr, int  word  ) {
    patch_value( addr, word,  2 );
}
void patch_long( int addr, long dword ) {
    patch_value( addr, dword, 4 );
}

void patch_word_be(int addr, int  word) {
    patch_value(addr, ((word & 0xFF00) >> 8) | ((word & 0x00FF) << 8), 2);
}

void append_byte( byte_t byte1 ) {
...
void append_long( long dword ) {
    append_value( dword, 4 );
}

void append_word_be(int  word) {
    append_value(((word & 0xFF00) >> 8) | ((word & 0x00FF) << 8), 2);
}
