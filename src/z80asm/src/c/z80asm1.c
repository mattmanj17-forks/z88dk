...
static void do_assemble(const char* src_filename, const char* obj_filename) {
    ...
    clear_error_location();
    Z80pass2(start_errors,
             obj_filename);	/* call pass 2 even if errors found, to issue pass2 errors */
    clear_error_location();
    list_close();
...
int z80asm_main() {
    ...
    if (!get_error_count()) {
        ...
        else if (option_is_consol_obj_file()
                 && option_files_size() > 1) {	// -o consolidated obj
            link_modules();
