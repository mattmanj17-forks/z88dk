...
void add_opcode_nn_nn(long long opcode0, long long opcode1,
                      struct Expr1* expr0) {
    // build expr1 = expr0
    UT_string* expr1_text;
    utstring_new(expr1_text);
...
void add_opcode_idx_idx1(long long opcode0, long long opcode1,
                         struct Expr1* expr0) {
    // build expr1 = 1+(expr)
    UT_string* expr1_text;
    utstring_new(expr1_text);
...
void add_opcode_jp_nnn_end(long long opcode, const char* end_label,
                           int offset) {
    UT_string* target;
    utstring_new(target);
    utstring_printf(target, "%s-%d", end_label, offset);
...
void add_copper_unit_wait(Expr1* ver, Expr1* hor) {
    ...
    if (cpu != CPU_Z80N && cpu != CPU_Z80N_STRICT) {
        ...
        snprintf(expr_text, sizeof(expr_text),
                 "0x8000 + (((%s) & 0x3F) << 9) + ((%s) & 0x1FF)", Str_data(hor->text),
                 Str_data(ver->text));
...
void add_copper_unit_move(Expr1* reg, Expr1* val) {
    ...
    if (cpu != CPU_Z80N && cpu != CPU_Z80N_STRICT) {
        ...
        char expr_text[MAXLINE];
        snprintf(expr_text, sizeof(expr_text),
                 "(((%s) & 0x7F) << 8) + ((%s) & 0xFF)", Str_data(reg->text),
                 Str_data(val->text));
        Expr1* expr = parse_expr(expr_text, false);
