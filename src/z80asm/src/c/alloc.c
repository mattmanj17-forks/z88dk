    ...
    while ( (block = next_to_free()) != NULL ) {
        ...
        if ( ! block->flags.destroy_atextit )
            log_warn("memory leak (%u bytes) allocated at %s:%d",
                     (unsigned)block->client_size, block->file ? block->file : "(null)",
                     block->lineno );
...
static MemBlock* new_block( size_t client_size, const char* file, int lineno ) {
    ...
    check( block,
           "memory alloc (%u bytes) failed at %s:%d", (unsigned)client_size, file,
           lineno );
...
*----------------------------------------------------------------------------*/
static void* m_alloc( size_t size, int fill, const char* source,
...
void* m_realloc_( void* memptr, size_t size, char* file, int lineno ) {
    ...
    check( block, "memory realloc (%u bytes) failed at %s:%d", (unsigned)size, file,
           lineno );
    ...
    if ( ! result ) {
        ...
        check( result, "memory fence check failed before realloc (%u bytes) at %s:%d",
               (unsigned)size, file, lineno );
    }
...
*   Destructors
*----------------------------------------------------------------------------*/
void* m_set_destructor_( void* memptr, destructor_t destructor, char* file,
                         int lineno ) {
    MemBlock* block;
...
void* m_set_in_collection_( void* memptr, bool in_collection, char* file,
                            int lineno ) {
    MemBlock* block;

    block = find_block( memptr, file, lineno );
    check( block, "m_%s_in_collection at %s:%d failed",
           in_collection ? "set" : "clear", file, lineno );
