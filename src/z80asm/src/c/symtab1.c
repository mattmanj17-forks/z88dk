...
void SymbolHash_cat( Symbol1Hash** ptarget, Symbol1Hash* source ) {
    ...
    for ( iter = Symbol1Hash_first( source ); iter;
...
*----------------------------------------------------------------------------*/
Symbol1* _define_sym(const char* name, long value, sym_type_t type,
                     sym_scope_t scope,
...
static Symbol1Hash* _select_module_symbols(Module1* module,
        bool(*cond)(Symbol1* sym)) {
...
void copy_static_syms( void ) {
    ...
    for ( iter = Symbol1Hash_first( static_symtab ); iter;
            iter = Symbol1Hash_next( iter ) ) {
        sym = ( Symbol1* )iter->value;
...
*----------------------------------------------------------------------------*/
static Symbol1* define_local_symbol(const char* name, long value,
                                    sym_type_t type) {
    Symbol1* sym;
    ...
    if (sym == NULL) {				/* Symbol1 not declared as local */
        /* create symbol */
        sym = Symbol_create(name, value, type, SCOPE_LOCAL, CURRENTMODULE,
                            CURRENTSECTION);
        sym->is_defined = true;
        Symbol1Hash_set(&CURRENTMODULE->local_symtab, name, sym);
    }
...
void declare_global_symbol(const char* name) {
    ...
    if (sym == NULL) {
        ...
        if (sym == NULL) {
            ...
            sym = Symbol_create(name, 0, TYPE_UNDEFINED, SCOPE_GLOBAL, CURRENTMODULE,
                                CURRENTSECTION);
...
void declare_public_symbol(const char* name) {
    ...
    if (sym == NULL) {
        ...
        if (sym == NULL) {
            /* not local, not global -> declare symbol as global */
            sym = Symbol_create(name, 0, TYPE_UNDEFINED, SCOPE_PUBLIC, CURRENTMODULE,
                                CURRENTSECTION);
            Symbol1Hash_set(&global_symtab, name, sym);
        }
...
void declare_extern_symbol(const char* name) {
    ...
    if (sym == NULL) {
        ...
        if (sym == NULL) {
            ...
            sym = Symbol_create(name, 0, TYPE_CONSTANT, SCOPE_EXTERN, CURRENTMODULE,
                                CURRENTSECTION);
            Symbol1Hash_set(&global_symtab, name, sym);
        }
        else if (sym->module == CURRENTMODULE && (sym->scope == SCOPE_PUBLIC
...
*----------------------------------------------------------------------------*/
static void _write_symbol_file(const char* filename, Module1* module,
                               bool(*cond)(Symbol1* sym),
    ...
    for (iter = Symbol1Hash_first(symbols); iter; iter = Symbol1Hash_next(iter)) {
        ...
        if (type_flag) {
            ...
            Str_append_sprintf(line, ", %s", (module == NULL
                                              && sym->module != NULL) ? sym->module->modname : "");
...
*----------------------------------------------------------------------------*/
static bool cond_all_symbols(Symbol1* sym) {
    return true;
}
...
static bool cond_global_symbols(Symbol1* sym) {
    return !(sym->is_global_def) && (sym->scope == SCOPE_PUBLIC
                                     || sym->scope == SCOPE_GLOBAL);
}
...
static const char* last_global_label =
    "";  // last non-local label, used as prefix for each new local label
...
static const char* local_labels_add_use(bool is_add, const char* short_name) {
    ...
    if (p == NULL) {                // standard label, no @
        ...
        if (*last_global_label == '\0') {
            ...
            utstring_printf(long_name, "%s%s", last_global_label,
                            short_name);  // label@label
