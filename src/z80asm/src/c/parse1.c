...
static void read_token(ParseCtx* ctx) {
    ...
    expr_start_index = (int)(ctx->expr_start ? ctx->expr_start -
                             (Sym*)utarray_front(ctx->tokens) : -1);
...
//	sym_copy.string = token_strings_add(sym.string);
    ...
    ctx->expr_start = expr_start_index >= 0 ? ((Sym*)utarray_front(
                          ctx->tokens)) + expr_start_index : NULL;
...
static void free_tokens(ParseCtx* ctx) {
    ...
    if (ctx->current_sm !=
            SM_DMA_PARAMS) {		// DMA_PARAMS needs to preserve exprs between lines
...
bool check_if_condition(Expr1* expr) {
    ...
    if (value ==
            0) {			// ignore expr->result.not_evaluable, as undefined values result in 0
        condition = false;
    }
    else {
        condition = true;
...
bool check_ifdef_condition(const char* name) {
    ...
    if (symbol != NULL && (symbol->is_defined || (symbol->scope == SCOPE_EXTERN
