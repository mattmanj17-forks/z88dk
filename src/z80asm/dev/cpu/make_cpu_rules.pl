...
sub parse_code {
    ...
    if ( $opcode->synth ) {
        push @code, "if (option_no_synth())", "\terror(ErrIllegalIdent, NULL);";
    }
    ...
    if ( $bytes =~ /%t/ ) {
        ...
        push @code, "asm_LABEL_offset(end_label, get_cur_opcode_size());", "}";
    }

    # handle multiple uses of the same expression
    elsif ( multiple_uses_of_expr($opcode) ) {
        push @code, "{", "DO_STMT_LABEL();", "Expr1 *expr = pop_expr(ctx);";
        for my $op (@ops) {
...
sub parse_code_opcode {
    ...
    if ( $bytes =~ s/ \@(\w+)// ) {
        my $func = $1;
        push @code, "DO_STMT_LABEL();", "add_call_emul_func(\"$func\");";
    }
    elsif ( $asm =~ /^mmu %c, %n/ ) {
        push @code,
...
sub merge_cpu {
        ...
        && scalar( keys %{ $code{$last_code} } ) == scalar(@cpus) )
    {
        ...
        $ret .= "default: " . "error(ErrIllegalIdent, NULL); " . "}\n";
    }
