...
{
    local $ENV{PATH} =
        join( $Config{path_sep}, "..", "../../../bin", $ENV{PATH} );
...
END
 ...
 # error for -r below 23760
    ...
    capture_nok( "z88dk-z80asm +zx -r" . ( $zx_rem_org - 1 ) . " -m test.asm",
        <<END );
...
END
 ...
 #------------------------------------------------------------------------------
    ...
    my $rem_P =
        ...
        pack( "v*", 9 ) .      # length
        pack( "C*", 234 ) .    # REM
        bin_code($zx81_rem_org) . pack( "C*", 118 ) .    # NEWLINE
...
END
 ...
 # no org, verbose
    ...
    capture_ok( "z88dk-z80asm +zx81 -m -v -L.. test.asm 2> ${test}.err",
        <<'END' );
