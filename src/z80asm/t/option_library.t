...
END
    ...
    delete $ENV{TEST_ENV};
    z80asm_ok( "-b -L${test}\${TEST_ENV}dir -l${test}${ext}",
        "", "", $asm, $bin );
}
...
capture_ok(
    "z88dk-z80asm " . "-x${test}1.lib ${test}1.asm ${test}2.asm ${test}3.asm",
    "" );
ok -f "${test}1.lib";
...
capture_ok(
    "z88dk-z80asm " . "-x${test}2.lib ${test}4.asm ${test}5.asm ${test}6.asm",
    "" );
ok -f "${test}2.lib";
...
check_bin_file( "${test}.bin",
    bytes( 13, 14, 15, 16, 17, 18, 10, 11, 12, 0, 7, 8, 9, 1, 2, 3, 4, 5, 6, )
);
