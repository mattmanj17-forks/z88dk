...
sub ticks {
    ...
    $ret->{C}    = shift @regs;
    $ret->{B}    = shift @regs;
    $ret->{BC}   = ( $ret->{B} << 8 ) | $ret->{C};
    $ret->{L}    = shift @regs;
    $ret->{H}    = shift @regs;
    $ret->{HL}   = ( $ret->{H} << 8 ) | $ret->{L};
    my $PCl = shift @regs;
    my $PCh = shift @regs;
    $ret->{PC} = ( $PCh << 8 ) | $PCl;
    my $SPl = shift @regs;
    ...
    $ret->{E}     = shift @regs;
    $ret->{D}     = shift @regs;
    $ret->{DE}    = ( $ret->{D} << 8 ) | $ret->{E};
    $ret->{C_}    = shift @regs;
    $ret->{B_}    = shift @regs;
    $ret->{BC_}   = ( $ret->{B_} << 8 ) | $ret->{C_};
    $ret->{E_}    = shift @regs;
    $ret->{D_}    = shift @regs;
    $ret->{DE_}   = ( $ret->{D_} << 8 ) | $ret->{E_};
    $ret->{L_}    = shift @regs;
    $ret->{H_}    = shift @regs;
    $ret->{HL_}   = ( $ret->{H_} << 8 ) | $ret->{L_};
    $ret->{F_}    = shift @regs;
    $ret->{F__S}  = ( $ret->{F_} & 0x80 ) ? 1 : 0;
...
sub pointers {
    return
        join( '', map { pack( "vC", $_ & 0xFFFF, ( $_ >> 16 ) & 0xFF ) } @_ );
}
...
sub objfile {
    ...
    my $name_addr = length($o);
    $o .= pack( "V", -1 );
    my $expr_addr = length($o);
    $o .= pack( "V", -1 );
    my $symbols_addr = length($o);
    $o .= pack( "V", -1 );
    my $lib_addr = length($o);
    $o .= pack( "V", -1 );
    my $code_addr = length($o);
    $o .= pack( "V", -1 );
    my $st_addr = length($o);
    $o .= pack( "V", -1 );
    ...
    if ( $args{EXPRS} ) {
        ...
        for ( @{ $args{EXPRS} } ) {
            @$_ == 9 or die;
            my (
                $type,        $filename,    $line_nr,
                $section,     $asmptr,      $ptr,
                $opcode_size, $target_name, $text
            ) = @$_;
            ...
            my %TYPES = (
                ...
                "S" => 3,
                "W" => 4,
                "C" => 4,    # was C until v17, after is W
                "B" => 5,
                "L" => 6,
                "u" => 7,
                "s" => 8,
    ...
    if ( $args{SYMBOLS} ) {
        ...
        for ( @{ $args{SYMBOLS} } ) {
            @$_ == 7 or die;
            my ( $scope, $type, $section, $value, $name, $def_filename,
                $line_nr )
                = @$_;
...
# Ticks: prepare tests and run all in one go
{
    ...
    sub add {
        ...
        for my $k ( sort keys %checks ) {
            ...
            if ( $k =~ /^F_S/ ) {
                $test_code .= $self->test_flag_code( $test_nr, $k, 0x80, $v );
            }
            elsif ( $k =~ /^F_Z/ ) {
                $test_code .= $self->test_flag_code( $test_nr, $k, 0x40, $v );
            }
            elsif ( $k =~ /^F_H/ ) {
                $test_code .= $self->test_flag_code( $test_nr, $k, 0x10, $v );
            }
            elsif ( $k =~ /^F_PV/ ) {
                $test_code .= $self->test_flag_code( $test_nr, $k, 0x04, $v );
            }
            elsif ( $k =~ /^F_N/ ) {
                $test_code .= $self->test_flag_code( $test_nr, $k, 0x02, $v );
            }
            elsif ( $k =~ /^F_C/ ) {
                $test_code .= $self->test_flag_code( $test_nr, $k, 0x01, $v );
            }
    ...
    sub _check_value {
        ...
        if ( $size == 1 ) {
            ...
            $got =
                ( $t->{mem}[$res_addr] + ( $t->{mem}[ $res_addr + 1 ] << 8 ) )
                & ( $mask ? $mask : 0xFFFF );
        }
        else {
            die;
    ...
    sub test_flag_code {
        ...
        push @{ $self->tests }, sub {
            ...
            local $Test::Builder::Level = $Test::Builder::Level + 1;
        SKIP: {
                skip "8085 does not have the N flag"
                    if $t->{cpu} eq '8085' && $flag eq 'F_N';
                $self->_check_value( $test_nr, $t, $flag, $res_addr, 1, $mask,
                    $value );
            }
        };
        return <<END;
...
END
    ...
    sub test_dd1_code {
        ...
        push @{ $self->tests }, sub {
            my ($t) = @_;
            local $Test::Builder::Level = $Test::Builder::Level + 1;
            $self->_check_value( $test_nr, $t, $dd_tick, $res_addr, 2, 0,
                $value );
        };
        return <<END;
						IF !__CPU_INTEL__ && !__CPU_GBZ80__
...
END
    ...
    sub test_r1_code {
        ...
        push @{ $self->tests }, sub {
            my ($t) = @_;
            local $Test::Builder::Level = $Test::Builder::Level + 1;
            $self->_check_value( $test_nr, $t, $r_tick, $res_addr, 1, 0,
                $value );
        };
        if ( $r =~ /A_/ ) {
            return <<END;
