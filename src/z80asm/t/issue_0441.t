...
my @output_lib = ( "${test}.o", "${test}.sym", "${test}.lis" );
my @output_bin = ( @output_lib, "${test}.map", "${test}.def", "${test}.bin" );
...
run_ok( "z88dk-z80asm -b -s -l -m -g " . "-O${test_dir}/obj ${test}.asm" );
check_bin_file( "${output_dir}/${test}.bin", $bin );
...
path("${test_dir}/src/s1/s2")->mkpath;
spew( "${test}.lis", "${test_dir}/**" );
capture_nok( "z88dk-z80asm -b -s -l -m -g " . quote_os("\@${test}.lis"),
    <<END );
