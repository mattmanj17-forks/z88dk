    ...
    )
{
        ...
        )
    {
        ...
        my $cond_instr =
            ( $instr eq "ret" ) ? "$instr $flag" : "$instr $flag, $var";
...
END
            bytes(
                $cond_opcode + 8 * $flag_code,
                ( $instr eq "ret" ) ? () : ( 0x34, 0x12 ),
                $uncond_opcode,
                ( $instr eq "ret" ) ? () : ( 0x34, 0x12 ),
                0,
                0,
...
for ( [ nz => 0 ], [ z => 1 ], [ nc => 2 ], [ c => 3 ] ) {
    ...
    z80asm_ok(
		...
		jr:
				...
				defw jr, $var
END
        bytes( 0x20 + 8 * $flag_code, 0x12 - 2, 0x18, 0x12 - 4, 0, 0, 0x12, 0 )
    );
}
    ...
    for ( [ nk => 0xDD ], [ k => 0xFD ], [ nx5 => 0xDD ], [ x5 => 0xFD ] ) {
        ...
        z80asm_ok(
			...
			$instr:
					...
					defw $instr, $var
END
            bytes( $opcode, 0x34, 0x12, 0, 0, 0x34, 0x12 )
        );
    }
}
