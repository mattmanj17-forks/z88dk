...
check_bin_file( "$test.bin",
    bytes( 0xE0, 0, 0xF0, 0, 0xE0, 0, 0xF0, 0, 0xE0, 1, 0xF0, 1 ) );

capture_ok( "z88dk-z80asm -mz80 -l -b $test.asm", "" );
check_bin_file(
    "$test.bin",
    bytes(
        0x32, 0,    0xFF, 0x3A, 0,    0xFF, 0x32, 0, 0xFF, 0x3A,
        0,    0xFF, 0x32, 1,    0xFF, 0x3A, 1,    0xFF
    )
);
...
capture_ok( "z88dk-z80asm -mgbz80 -l -b $test.asm", "" );
check_bin_file( "$test.bin", bytes( 0xE2, 0xF2 ) );
